...
function getGitHubPagesFileUrlForRecord(record = {}) {
  const directUrl = record.html_url || record.file_url;
  if (directUrl) {
    try {
      const parsed = new URL(String(directUrl));
      if (['http:', 'https:'].includes(parsed.protocol)) return parsed.href;
    } catch {
      // Rebuild the public URL below when the stored value is missing or invalid.
    }
  }
  ...
  const path = buildGitHubPagesFilePath({
    ...
    preferredFilename: record.filename || record.name,
    existingUrl: record.file_url || record.html_url || null,
    existingPath: record.file_data || null,
    userId: record.supabase_user_id || record.user_id || null,
  });

  return buildGitHubPagesFileUrl({
    owner: config.owner,
    repo: config.repo,
...
module.exports = {
  ...
  getGitHubPagesFileUrlForRecord,
