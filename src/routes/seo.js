...
const { updateSeoMetadata } = require('../utils/seoHtml');
...
router.get('/categorias/:category', async (req, res, next) => {
  try {
    ...
    html = updateSeoMetadata(html, { title, description, image, canonicalUrl });
...
router.get('/archivos/:slug/:id', async (req, res, next) => {
  try {
    ...
    const canonicalUrl = `${req.protocol}://${req.get('host')}${req.path}`;
    // fetch file record
    ...
    if (!error && data) {
      const fileName = String(data.name || data.filename || '').trim().replace(/\.html?$/i, '');
      title = fileName ? `${fileName} | SysteCode` : title;
