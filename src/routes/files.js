...
const { buildGitHubPagesFilePath, buildGitHubPagesFileUrl, buildStorageHtmlPath, getGitHubPagesFileUrlForRecord } = require('../utils/githubPages');
...
router.get('/files', async (req, res) => {
  try {
    ...
    const mapped = (data || []).map((rec) => {
      ...
      const html_url = sanitizeUrl(getGitHubPagesFileUrlForRecord(rec) || rec.supabase_url || null);
...
router.get('/file/:id/download', async (req, res) => {
  try {
    ...
    const githubPagesUrl = getGitHubPagesFileUrlForRecord(rec);
    const safeAbsoluteUrls = [githubPagesUrl, rec.html_url, rec.file_url, rec.supabase_url]
      .filter(Boolean)
